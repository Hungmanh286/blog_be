...
SHEET_CONFIG = {
    ...
    'Vol': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'Volatility Index': 'volatility_index'}},
    'Bre': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'Breadth Index': 'breadth_index'}},
    'Map': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'Market price': 'market_price'}},
    'Div': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'% Dividen 12M': 'dividend_12m_pct'}},
    'Eps': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'Vnindex': 'eps_vnindex', 'Nonbank': 'eps_nonbank'}},
    'PB': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'Vnindex': 'pb_vnindex', 'Non bank': 'pb_nonbank', 'Bank': 'pb_bank'}},
    '%St': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'% PE < 10': 'st_pe_under_10_pct', '% PB < 1': 'st_pb_under_1_pct'}},
    'Ret': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'40 Years Old': 'ret_40years_old_pct', 'VNI Adjusted': 'ret_vni_adjusted_pct'}},
    'Tur': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'Turnover ratio': 'turnover_ratio'}},
    'Der': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'Derivaties ratio': 'derivatives_ratio'}},
    'Ins': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'% Insider Transaction 3M': 'insider_transaction_3m_pct'}},
    'Tra': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'Probability': 'probability'}},
    'Avg': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'Avg 50D Orders': 'avg_50d_orders'}},
    'Mat': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'Matching Rate (%)': 'matching_rate_pct'}},
    'Cor': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'SPX': 'cor_spx', 'VN1Y': 'cor_vn1y', 'USD': 'cor_usd'}},
    'Hea': {'table': 'market_indicators', 'date_col': 'Date', 'mapping': {'Consumption': 'hea_consumption', 'Production': 'hea_production', 'Labor': 'hea_labor'}},
    
    # === PORTFOLIO_PERFORMANCE (Hiệu suất danh mục theo năm) ===
    'Por': {'table': 'portfolio_performance', 'year_col': 'Year'},
    
    # === SECTOR_VALUATION (Định giá theo ngành) ===
    'Sec': {'table': 'sector_valuation', 'entity_col': 'Ngành', 'mapping': {'PE percentile': 'pe_percentile', 'PB percentile': 'pb_percentile'}},
    
    # === WORLD_MARKET_ANALYSIS (Phân tích thị trường thế giới) ===
    'Wma': {'table': 'world_market_analysis', 'entity_col': 'Unnamed: 0', 'mapping': {'YTD (%)': 'ytd_pct', 'Percentile Growth ': 'percentile_growth', 'Percentile Valuation': 'percentile_valuation'}},
    
    # === MACRO_INDICATORS (Chỉ số vĩ mô theo tháng) ===
    '% GDP': {'table': 'macro_indicators', 'date_col': ' vb', 'mapping': {'% GDP': 'gdp_growth_pct', '% M2': 'm2_growth_pct'}},
    'Mar': {'table': 'macro_indicators', 'date_col': 'Unnamed: 0', 'mapping': {'Margin': 'mar_margin', 'Deposit': 'mar_deposit'}},
}
        ...
        try:
            ...
            for sheet_name in excel_file.sheet_names:
                ...
                try:
                    ...
                    logger.info(f"Processing sheet '{sheet_name}' -> table '{config['table']}', rows: {len(df)}")
                    ...
                    logger.info(f"Sheet '{sheet_name}' processed successfully: {records_count} records")
    ...
    def _process_portfolio_performance(self, df: pd.DataFrame, config: Dict) -> int:
        ...
        for _, row in df.iterrows():
            try:
                ...
                values_placeholders = ', '.join([f":{c}" for c in columns])
                update_clause = ', '.join([f"{k} = EXCLUDED.{k}" for k in values.keys()])
