...
SHEET_COLUMN_MAPPING = {
    "Vol": {"date_col": "Date", "columns": {"Volatility Index": "volatility_index"}},
    "Bre": {"date_col": "Date", "columns": {"Breadth Index": "breadth_index"}},
    "Map": {"date_col": "Date", "columns": {"Market price": "market_price"}},
    "Div": {"date_col": "Date", "columns": {"% Dividen 12M": "dividend_12m_pct"}},
    "Eps": {
        "date_col": "Date",
        "columns": {"Vnindex": "eps_vnindex", "Nonbank": "eps_nonbank"},
    },
    "%St": {
        "date_col": "Unnamed: 0",
    ...
    "Tur": {"date_col": "Date", "columns": {"Turnover ratio": "turnover_ratio"}},
    "Der": {"date_col": "Date", "columns": {"Derivaties ratio": "derivatives_ratio"}},
    "Ins": {
        "date_col": "Date",
        "columns": {"% Insider Transaction 3M": "insider_transaction_3m_pct"},
    },
    "Tra": {"date_col": "Date", "columns": {"Probability": "probability"}},
    "Avg": {"date_col": "Date", "columns": {"Avg 50D Orders": "avg_50d_orders"}},
    "Mat": {"date_col": "Date", "columns": {"Matching Rate (%)": "matching_rate_pct"}},
    "Cor": {
        "date_col": "Date",
        "columns": {"SPX": "cor_spx", "VN1Y": "cor_vn1y", "USD": "cor_usd"},
    },
    "Hea": {
        "date_col": "Date",
        "columns": {
    ...
    "%GDP": {"date_col": "Date", "columns": {"% GDP": "gdp_pct", "% M2": "m2_pct"}},
    "Mar": {"date_col": "Date", "columns": {"Margin": "margin", "Deposit": "deposit"}},
}
    ...
    def _merge_all_sheets(self, excel_file: pd.ExcelFile) -> pd.DataFrame:
        ...
        for sheet_name in excel_file.sheet_names:
            ...
            try:
                ...
                logger.info(
                    f"Sheet '{sheet_name}': {len(df)} rows, date range: {sheet_min.date()} to {sheet_max.date()}"
                )
    ...
    def _insert_to_db(self, df: pd.DataFrame) -> int:
        ...
        for _, row in df.iterrows():
            try:
                ...
                logger.warning(
                    f"Error inserting row for date {row.get('report_date')}: {e}"
                )
                continue
