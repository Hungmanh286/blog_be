...
class WorldMarketAnalysis(Base):
    ...
    country = Column(String(100), primary_key=True)  # wma (country)
    ytd_pct = Column(Float)                          # ytd(%)
    percentile_growth = Column(Float)                # percentile growth
    percentile_valuation = Column(Float)             # percentile valuation
